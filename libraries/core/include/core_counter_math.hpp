...
public:
    ...
    /// Get current value
    CORE_INLINE T ToUnsigned() const
    {
        return Value;
    }
    ...
    /// Post-increment
    CORE_INLINE ThisType operator++(int)
    {
    ...
    CORE_INLINE ThisType& operator+=(const ThisType b)
    {
        Value = (Value + b.Value) & kMask;
        return *this;
    }

    CORE_INLINE ThisType operator+(const ThisType b) const
    {
    ...
    CORE_INLINE bool operator==(const ThisType b) const
    {
    ...
    */
    ...
    CORE_INLINE SmallerT Truncate() const
    {
    ...
    template<class SmallerT>
    static CORE_INLINE ThisType ExpandFromTruncatedWithBias(
    ...
    template<class SmallerT>
    static CORE_INLINE ThisType ExpandFromTruncated(
        const ThisType recent,
        const SmallerT smaller)
    {
        static_assert(SmallerT::kBits < kBits, "Smaller type must be smaller");
...
*/
CORE_INLINE Counter64 CounterExpand(
    uint64_t largest,
    uint64_t partial,
    unsigned bytes)
{
