    ...
    bool bUnAlignedDeviceCopy)
{
    ...
    uint32_t srcPitch = nSrcPitchY ? nSrcPitchY : NvEncoder::GetWidthInBytes(pixelFormat, width);
    CUDA_MEMCPY2D m = { 0 };
    m.srcMemoryType = srcMemoryType;
    if (srcMemoryType == CU_MEMORYTYPE_HOST)
    {
...

    {
        if (chromaHeight)
        {
            ...
            {
                m.srcDevice = (CUdeviceptr)pSrcU;
            }
            m.srcPitch = srcChromaPitch;

            m.dstDevice = dstChromaDevicePtrs[0];
    ...
    }
    {
        if (chromaHeight)
        {
            ...
            {
                m.srcDevice = (CUdeviceptr)pSrcV;
            }
            m.srcPitch = srcChromaPitch;

            m.dstDevice = dstChromaDevicePtrs[1];
            m.dstPitch = dstChromaPitch;
            m.WidthInBytes = chromaWidthInBytes;
            m.Height = chromaHeight;
            if (bUnAlignedDeviceCopy && srcMemoryType == CU_MEMORYTYPE_DEVICE)
